"""
Раскодирование JSON-эскейпов ради анализа текста.

`json.dumps` без `ensure_ascii=False` превращает кириллицу в `\\uXXXX`, и в такой строке
ни анализатор, ни правила ничего не находят: букв в ней нет. Раскодированный текст идёт
на разбор, а найденные спаны переносятся обратно в координаты исходной строки — маска
встаёт в неё, и формат остаётся прежним.
"""
...
# Дешёвая проверка до всякой работы: без \uXXXX раскодировать нечего.
ESCAPE_TRIGGER = "\\u"
...
class DecodedText:
    """Раскодированный текст и карта его позиций в исходную строку."""

    text: str
    # Карта хранится отрезками, а не по символу на запись: отрезков столько, сколько эскейпов,
    # а внутри отрезка позиции идут подряд.
